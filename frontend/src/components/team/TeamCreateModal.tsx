import { useState } from 'react';
import { createPortal } from 'react-dom';
import { X } from 'lucide-react';
import { teamApi } from '@/services/teamApi';
import { refreshTeams, useTeamStore } from '@/stores/teamStore';
import { Button } from '@/components/ui/button';

interface Props {
  onClose: () => void;
  onCreated: (teamId: string) => void;
}

export function TeamCreateModal({ onClose, onCreated }: Props) {
  const [name, setName] = useState('');
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState('');

  const handleCreate = async () => {
    const trimmed = name.trim();
    if (!trimmed) {
      setError('请输入团队名称');
      return;
    }
    setSubmitting(true);
    setError('');
    try {
      const team = await teamApi.createTeam(trimmed);
      await refreshTeams();
      useTeamStore.getState().setActiveTeamId(team.id);
      onCreated(team.id);
    } catch (e: any) {
      setError(e?.message || '创建失败');
      setSubmitting(false);
    }
  };

  return createPortal(
    <div
      className="fixed inset-0 z-[1200] flex items-center justify-center bg-black/30 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
        className="relative w-full max-w-sm mx-4 rounded-3xl bg-white shadow-[0_32px_80px_rgba(15,23,42,0.18)] border border-slate-200/80 p-6"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center hover:bg-slate-100 transition-colors text-slate-400"
        >
          <X className="w-4 h-4" />
        </button>

        <h2 className="text-base font-semibold text-slate-800 mb-4">新建团队</h2>

        <input
          autoFocus
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder="团队名称"
          disabled={submitting}
          className="w-full rounded-xl border border-sky-300 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-sky-200"
          onKeyDown={(e) => {
            if (e.key === 'Enter') void handleCreate();
          }}
        />
        <p className="mt-2 text-xs text-slate-400">新建团队固定 2 席位起</p>

        {error && <p className="mt-2 text-xs text-red-500">{error}</p>}

        <div className="mt-6 flex items-center gap-3">
          <Button
            className="flex-1 rounded-full bg-slate-800 hover:bg-slate-900"
            disabled={submitting}
            onClick={() => void handleCreate()}
          >
            {submitting ? '创建中…' : '创建'}
          </Button>
          <button
            type="button"
            className="text-sm text-slate-600 hover:text-slate-900 px-2"
            disabled={submitting}
            onClick={onClose}
          >
            取消
          </button>
        </div>
      </div>
    </div>,
    document.body,
  );
}
