const enUS = {
  common: {
    language: "Language",
    help: "Help",
    settings: "Settings",
    untitled: "Untitled",
    user: "User",
    lang: {
      zh: "ZH",
      en: "EN",
    },
    actions: {
      login: "Log in",
      register: "Sign up",
      logout: "Log out",
      save: "Save",
      send: "Send",
      back: "Back",
    },
    status: {
      online: "Online",
      refreshed: "Refreshed",
      mock: "Mock",
      unknown: "Unknown",
    },
  },
  home: {
    wechat: {
      followOfficial: "Follow Official Account",
      taiOfficialAccount: "TAI Official Account",
      joinGroup: "Join WeChat Group",
      taiLearningGroup: "TAI Learning Group",
      noImage: "No image",
    },
    header: {
      greeting: "Hi, {{name}}",
      nav: {
        toolbox: "Toolbox",
        scenes: "Scene Library",
        cocreate: "Co-create",
      },
      actions: {
        login: "Log in",
        register: "Sign up",
        logout: "Log out",
        membership: "Membership",
        myCredits: "My Credits",
        referral: "Invite & Earn",
      },
    },
    hero: {
      logoAlt: "TAI — More than AI",
      titleSuffix: "More than AI",
      subtitle: "TAI — a next-generation intelligent creative platform focused on architecture and education, using AI to understand industries and unlock creativity.",
      startNow: "Get Inspired",
      architectureIndustry: "Architecture",
      otherIndustry: "Other Industries",
    },
    features: {
      title: "Powerful Creation Tools",
      aiTitle: "AI-Powered Creation",
      aiDesc: "Use AI to generate creative content quickly.",
      drawingTitle: "Professional Drawing",
      drawingDesc: "Node-based workflows for flexible tool composition.",
      styleTitle: "Diverse Styles",
      styleDesc: "Support for multiple artistic styles and needs.",
      items: [
        { label: "Intelligent Generation", desc: "AI-powered design inspiration" },
        { label: "Efficient Collaboration", desc: "Real-time team collaboration" },
        { label: "Precision & Professional", desc: "Details make quality" },
        { label: "Continuous Evolution", desc: "Every step forward" },
      ],
      cards: [
        {
          category: "CONSTRUCTION INDUSTRY",
          title: "Architecture",
          desc: "Full-discipline AI support in one click",
        },
        {
          category: "EDUCATIONAL TRAINING",
          title: "Education & Training",
          desc: "Upgrade your classroom with immersive teaching",
        },
        {
          category: "CREATIVE DESIGN",
          title: "Creative Design",
          desc: "Spark inspiration and craft one-of-a-kind creative work",
        },
      ],
    },
    cta: {
      ready: "Ready to get started?",
      desc: "Try the AI assistant now and begin your creative journey.",
      start: "Start Creating",
    },
    eventModal: {
      close: "Close",
      action: "Learn More",
      prev: "Previous slide",
      next: "Next slide",
      goToSlide: "Go to slide {{index}}",
      imageAlt: "Event image {{index}}",
    },
    videoUnsupported: "Your browser does not support video playback.",
    linkCopied: "Link copied to clipboard!",
    shareLink: "Share link: {{url}}",
    share: {
      title: "TAI",
      text: "Try this intelligent canvas app!",
    },
    icp: "ICP License: 粤ICP备2023120022号",
    toolbox: {
      title: "Toolbox",
      subtitle: "Powerful AI-driven features that make creation smarter and more efficient",
      prev: "Previous",
      next: "Next",
      items: [
        {
          title: "Infinite Canvas",
          titleEn: "Infinite Canvas",
          desc: "Free zoom and pan with a borderless creative space and multi-level content organization.",
          duration: "02:36",
          icon: "wxhb",
        },
        {
          title: "Chatbot Assistant",
          titleEn: "Chatbot Assistant",
          desc: "A multi-mode conversational creative assistant that turns design ideas into reality in one click.",
          duration: "02:36",
          icon: "chat",
        },
        {
          title: "Smart Nodes",
          titleEn: "Smart Nodes",
          desc: "An AI node-based generation platform with custom node connections for flexible design and workflow orchestration.",
          duration: "02:36",
          icon: "znjd",
        },
        {
          title: "Collaboration",
          titleEn: "AI Assistant",
          desc: "An AI-powered smart node system that connects context automatically for efficient information flow.",
          duration: "02:36",
          icon: "xthz",
        },
      ],
    },
    ctaBanner: {
      tagline: "AI empowers industries · Unlimited creative possibilities",
    },
    footer: {
      brandSuffix: "Tiangong · Shenjiang",
      slogan: "Understand industries with AI, unlock creativity, and empower every creator",
      products: "Products",
      dfc: "DFC",
      support: "Support",
      helpCenter: "Help Center",
      docs: "Documentation",
      contact: "Contact Us",
      follow: "Follow Us",
      copyright: "© 2024 TAI All rights reserved.",
    },
    scenes: {
      title: "Scene Library",
      subtitle: "Covering architecture, education, and creative design to empower diverse creative needs",
      more: "More Scenes",
      filters: {
        all: "All",
        architecture: "Urban Planning",
        education: "Architectural Design",
        career: "Interior Space",
        skills: "Landscape Design",
        online: "Education & Training",
        corporate: "Creative Design",
      },
      items: [
        { title: "Urban Renewal", imageKey: "城市更新", category: "architecture" },
        { title: "Urban Design", imageKey: "城市设计", category: "architecture" },
        { title: "Concept Scheme", imageKey: "概念方案", category: "education" },
        { title: "Building Renovation", imageKey: "建筑改造", category: "education" },
        { title: "Rendering", imageKey: "效果图渲染", category: "education" },
        { title: "Spatial Design", imageKey: "空间设计", category: "education" },
        { title: "Seasonal Transformation", imageKey: "时节变换", category: "education" },
        { title: "Chinese Home Design", imageKey: "中式家装", category: "career" },
        { title: "Hotel Space", imageKey: "酒店空间", category: "career" },
        { title: "Furniture Design", imageKey: "家具设计", category: "career" },
      ],
    },
  },
  auth: {
    modal: {
      expiredTitle: "Session Expired",
      expiredHint: "Your login session has expired. Please log in again to continue. Current page content will not be lost.",
      relogin: "Log in Again",
      loginFailed: "Login failed. Please try again.",
    },
    agreements: {
      mustAgree: "Please agree to the Terms and Privacy Policy first.",
      prefix: "I have read and agree to",
      terms: "Terms of Service",
      privacy: "Privacy Policy",
      community: "Community Guidelines",
      and: "and",
      comma: ",",
      consentModalLead: "To better protect your rights, please read and agree to ",
      consentModalTail: " so we can look up or create your account and provide related benefits and services.",
      consentModalAgree: "Agree and Continue",
      consentModalDisagree: "Disagree",
    },
    videoUnsupported: "Your browser does not support video playback.",
    login: {
      welcome: "Welcome",
      passwordTab: "Password",
      smsTab: "SMS Code",
      phonePlaceholder: "Enter phone number",
      passwordPlaceholder: "Enter password",
      codePlaceholder: "Enter verification code",
      forgotPassword: "Forgot password",
      registerNow: "Create account",
      submit: "Log in",
      submitting: "Logging in...",
      sendCode: "Send",
      resendCode: "Resend ({{seconds}}s)",
      smsSent: "Code sent. Please check your SMS and enter it manually.",
      phoneRequired: "Please enter phone number",
      phoneInvalid: "Invalid phone number format",
      otherMethods: "Other methods",
      watchaName: "Watcha",
      thirdPartyDivider: "Third-party login",
      watchaSubmit: "Continue with Watcha",
      wechatTitle: "WeChat Login",
      wechatScanAlt: "WeChat QR login code",
      wechatHint: "Open WeChat, scan the code, and follow the account to log in.",
      wechatLoading: "Generating QR code",
      wechatUnavailable: "QR code unavailable",
      wechatRefreshing: "Refreshing...",
      wechatRefresh: "Refresh QR Code",
      wechatExpired: "QR code expired. Refresh and try again.",
      wechatAuthorizing: "Scanned. Signing you in...",
      wechatBinding: "Verifying phone number and completing sign-in...",
      wechatRegisterTitle: "Complete Registration",
      wechatBindHint: "New WeChat user detected. Please fill in the registration form to create your account.",
      wechatBindHintWithName: "Hi {{name}}, please complete the registration form to create your account.",
      wechatBindSubmit: "Finish Registration",
      wechatBindSubmitLoading: "Submitting...",
      wechatBindIncomplete: "Please enter your phone number and verification code.",
      wechatBindPasswordRequired: "Please set a password with at least 6 characters.",
      wechatBindFailed: "Registration failed. Please try again.",
      wechatLoadFailed: "Failed to load WeChat QR login",
      identityEnterprise: "Enterprise",
      identityPersonal: "Personal",
      identityLinglong: "Linglong Member",
      enterpriseWelcome: "Enterprise Sign In",
      linglongWelcome: "Linglong Member Sign In",
      enterpriseNoAccess: "This account has no enterprise access. Please use an enterprise account.",
    },
    register: {
      title: "Create Account",
      subtitle: "Join TAI and start your design journey",
      phonePlaceholder: "Phone number",
      emailPlaceholder: "Email (optional)",
      namePlaceholder: "Username (required)",
      passwordPlaceholder: "Set password (at least 6 chars)",
      confirmPlaceholder: "Confirm password",
      invitePlaceholder: "Invite code (optional)",
      submit: "Sign up",
      submitting: "Submitting...",
      hasAccount: "Already have an account?",
      goLogin: "Log in",
      inviteFrom: "Invited by {{name}}",
      passwordMismatch: "Passwords do not match",
      invalidInvite: "Invalid invite code. Please check and try again.",
      usernameRequired: "Please enter a username",
      usernameCannotMatchEmail: "Username cannot be the same as email",
      usernameCannotMatchPhone: "Username cannot be the same as phone number",
      phoneInvalid: "Please enter a valid phone number",
      codeInvalid: "Please enter a 6-digit verification code",
      sendFailed: "Failed to send",
      codeCountdown: "Resend in {{count}}s",
      resend: "Resend",
      send: "Send",
    },
    linglongRegister: {
      heroTitle: "Become a Linglong Ecosystem Member",
      heroSubtitle: "Unlock exclusive supply chain assets, templates, and service channels",
      featureSupplyChain: "Exclusive supply chain library · DFC classification",
      featureTemplate: "Exclusive templates · One-click AI renders",
      featureService: "Human + AI dual-channel service window",
      featureIadu: "IADU team collaboration support",
      formTitle: "Linglong Member Registration",
      formSubtitle: "Join the Linglong private ecosystem with an invitation code",
      inviteCodePlaceholder: "Invitation code (required)",
      namePlaceholder: "Full name (required)",
      companyPlaceholder: "Company (required)",
      passwordSetPlaceholder: "Password (min. 6 characters)",
      confirmPasswordPlaceholder: "Confirm password",
      smsResendCountdown: "Resend in {{count}}s",
      inviteRequired: "Invitation code is required",
      nameRequired: "Name is required",
      companyRequired: "Company is required",
      submit: "Sign Up",
      goLogin: "Log in",
      forgotPassword: "Forgot password",
    },
  },
  workspace: {
    header: {
      help: "Help",
      userManual: "User Manual",
      pricingCatalog: "Pricing Catalog",
      settings: "Settings",
      referral: "Invite & Earn",
      myCredits: "My Credits",
      memberCountWithRole: "{{count}} members · {{role}}",
      backHome: "Back to Home",
      renameHint: "Double-click to rename",
      switchProject: "Switch Project",
      noProjects: "No projects",
      openManageFile: "Open/Manage Files",
      newProject: "New Project",
      openLibrary: "Open Library",
      closeLibrary: "Close Library",
      library: "Library",
      globalAutoRun: "Global Auto Run",
      globalAutoRunning: "Global Run in Progress...",
      globalAutoStop: "Terminate",
      routeSelector: {
        title: "Text/Image Route",
      },
    },
    appearance: {
      languageTitle: "Interface Language",
      languageDesc: "Switch between Chinese and English",
    },
    checkInReminder: {
      message: "Check in daily for 50 credits — 7-day streak total",
      messageWithStreak: "{{count}}-day streak — 7-day total",
      creditsSuffix: "credits",
      action: "Check in",
      dismiss: "Dismiss reminder",
    },
    channelSourceReminder: {
      message: "Share your source channel and get",
      messageTitle: "Share your source channel and get 100 credits",
      creditsSuffix: "credits",
      action: "Claim now",
      dismiss: "Dismiss reminder",
      modalTitle: "Choose your source channel",
      modalDesc: "Tell us how you found Tanva and claim",
      cancel: "Cancel",
      confirm: "Confirm claim",
      submitting: "Claiming…",
      closeModal: "Close",
      claimFailed: "Claim failed. Please try again later.",
      networkError: "Network error. Please try again later.",
    },
    settings: {
      title: "Settings",
      sections: {
        workspace: "Workspace",
        profile: "Complete Profile",
        referral: "Referral",
        appearance: "Appearance",
        ai: "AI Settings",
        advanced: "Advanced",
      },
      workspaceTab: {
        greeting: "Hi, {{name}}",
        loading: "Loading...",
        openManageFile: "Open/Manage Files",
        backHome: "Back to Home",
        globalHistory: "Global Image History",
        clearCanvas: "Clear Canvas",
        clearCanvasConfirm:
          "Are you sure you want to clear all canvas content?\nThis removes all drawing elements and nodes (keeps background/grid) and currently cannot be undone.",
        clearCanvasFailed: "Failed to clear canvas. Please try again later.",
        history: {
          empty: "No image history to clear.",
          clearConfirm:
            "Are you sure you want to clear {{count}} image history records? This only clears local cache and will not delete cloud files.",
        },
        dailyReward: {
          success: "Claimed successfully: daily login reward has been granted.",
          alreadyClaimed: "Today's reward has already been claimed.",
          failed: "Failed to claim reward. Please try again later.",
        },
        credits: {
          title: "My Credits",
          detail: "Credit Details",
          unit: "credits",
          recharge: "Top Up Now",
          empty: "No credit info available.",
        },
      },
      profileEditor: {
        editButton: "Edit",
        title: "Edit profile",
        subtitle: "Update your username and avatar",
        avatarLabel: "Avatar",
        avatarHint: "Upload an image for your avatar",
        removeAvatar: "Remove avatar",
        nameLabel: "Username",
        cancel: "Cancel",
        save: "Save",
        saving: "Saving...",
        uploading: "Uploading avatar...",
        uploadFailed: "Failed to upload avatar. Please try again.",
        saveFailed: "Failed to save profile. Please try again.",
        invalidImage: "Please choose an image file",
      },
      referralTab: {
        alerts: {
          checkInSuccess: "Check-in successful. You earned {{reward}} credits{{bonus}}",
          checkInWeeklyBonus: " (includes +100 bonus on day 7)",
          checkInFailed: "Check-in failed. Please try again later.",
        },
        timeAgo: {
          minutes: "{{count}} minutes ago",
          hours: "{{count}} hours ago",
          days: "{{count}} days ago",
        },
        checkIn: {
          title: "Daily Check-in",
          consecutive: "{{count}}-day streak",
          checkingIn: "Checking in...",
          checkInNow: "Check in now +{{reward}}",
          checkedToday: "Checked in today",
          weeklyBonusHint:
            "50 credits per day; +100 on day 7 — 450 credits in 7 days",
        },
        stats: {
          successfulInvites: "Successful Invites",
          totalEarnings: "Total Earnings",
        },
        inviteCode: {
          title: "My Invite Code",
          copy: "Copy",
          copied: "Copied",
          desc: "Share your invite code to bring in friends. Both sides receive bonus credits.",
        },
        status: {
          title: "Invite Status",
          subtitle: "INVITE STATUS",
          ruleLabel: "Reward rule:",
          ruleDesc: "Earn referral credits each time an invited friend completes their first creation. There is no limit on how many invites can earn rewards.",
          invited: "Invited",
          rewarded: "Rewarded",
          pendingFirstGeneration: "Pending first creation",
          empty: "No invite records yet.",
        },
      },
      appearanceTab: {
        saveView: {
          title: "Save View",
          desc: "Save current grid style and colors, and keep them after refresh.",
          button: "Save Settings",
          saved: "Saved",
          saveFailed: "Save failed",
        },
        gridRender: {
          title: "Grid Rendering",
          desc: "Show background reference lines",
        },
        snap: {
          title: "Snap Alignment",
          desc: "Intelligently align elements",
        },
        style: {
          title: "Style",
          grid: "Grid",
          solid: "Solid",
        },
        gridUnit: {
          title: "Grid Unit",
        },
        zoom: {
          title: "Zoom Sensitivity",
          desc: "Adjust damping feel",
        },
        gridColor: {
          title: "Grid Color",
        },
        edgeColorMode: {
          title: "Edge Color",
          desc: "Set how flow edge colors are displayed",
          standard: "Standard",
          handle: "Follow Handle",
        },
        chatStyle: {
          title: "AI Chat Style",
          transparent: "Transparent",
          solid: "Solid",
        },
      },
      aiTab: {
        imageOnly: {
          title: "Image-only Mode",
          desc: "Disable text output and only return images",
        },
        shortcuts: {
          title: "Send Shortcut",
          desc: "Choose your preferred send behavior in chat input.",
          enterLabel: "Press Enter",
          enterDesc: "Enter to send, Shift+Enter for newline",
          modEnterDesc: "Enter for newline, Ctrl/Cmd + Enter to send",
        },
        wheel: {
          title: "Mouse Wheel",
          desc: "Swap zoom and pan behavior between wheel and Ctrl/Cmd + wheel.",
          modifierLabel: "Default Mode",
          modifierDesc: "Wheel pans, Ctrl/Cmd + wheel zooms",
          directLabel: "Reversed Mode",
          directDesc: "Wheel zooms, Ctrl/Cmd + wheel pans",
        },
        provider: {
          title: "AI Provider",
          banana: "Domestic Pro",
          bananaDesc: "2nd-gen model with best quality; avoid peak hours",
          banana31: "Domestic Ultra",
          banana31Desc: "Latest model with higher quality",
          banana25: "Domestic Fast",
          banana25Desc: "1st-gen model, fast and stable",
        },
        bananaRoute: {
          title: "Nano Banana/Gemini Text+Image Route",
          desc: "Use one route policy for both text and image generation",
          normal: "Normal Route",
          normalDesc: "Image/text use normal third-party channels (admin controlled)",
          stable: "Premium Route",
          stableDesc: "Image/text use direct channel (stability first)",
          hint: "Only applies when Nano Banana providers are selected",
        },
        googleKey: {
          clearConfirm:
            "Are you sure you want to clear your custom API key? The system will switch back to the official key.",
        },
      },
      advancedTab: {
        openPanel: "Open Panel",
        closePanel: "Close Panel",
        memory: {
          title: "Memory Monitor",
          desc: "Debug tool available only in development mode",
        },
        history: {
          title: "History Debug",
          desc: "Inspect undo/redo stacks and snapshot details",
        },
        sandbox: {
          title: "Paper.js Sandbox",
          desc: "Open the Paper.js code debugging workspace",
          open: "Open Sandbox",
        },
        logout: {
          title: "Log Out",
          desc: "Sign out the current account and return to login page",
          button: "Log Out",
          loading: "Logging out…",
          confirm: "Are you sure you want to log out?",
        },
        changePassword: {
          title: "Change Password",
          desc: "Set a new login password with an SMS verification code",
          button: "Change Password",
        },
      },
    },
  },
  creditsPage: {
    loading: "Loading...",
    title: "My Credits",
    chartNoData: "No data",
    alerts: {
      dailyRewardSuccess: "Claimed successfully: daily login reward has been granted.",
      dailyRewardAlreadyClaimed: "Today's reward has already been claimed.",
      dailyRewardFailed: "Failed to claim reward. Please try again later.",
    },
    summary: {
      available: "Available Credits",
      earned: "Total Earned",
      spent: "Total Spent",
      todaySpent: "Spent Today",
      latestGenerationTime: "Latest generation time",
      noGenerationYet: "No generation record",
    },
    expiring: {
      title: "You have {{total}} credits expiring soon",
      desc: "Check-in credits expire after 7 days. Use them soon. After upgrading to paid, check-in credits never expire.",
      detail: "{{amount}} credits will expire on {{date}}",
      more: "{{count}} more expiring soon...",
    },
    paidUserBadge: "Paid user - check-in credits never expire",
    tabs: {
      overview: "Overview",
      transactions: "Credit Records",
      usage: "API Usage",
    },
    overview: {
      trendTitle: "Spending Trend (Last 14 Days)",
      trendWeekSpent: "Last 7 days spent: {{value}} credits",
      serviceStatsTitle: "Service Usage",
      noUsage: "No usage records",
      usageItem: "{{count}} runs / {{credits}} credits",
    },
    transactions: {
      title: "Credit Records",
      empty: "No credit records yet",
      generationTime: "Generated at",
      recordTime: "Recorded at",
      columns: {
        item: "Item",
        status: "Status",
        amount: "Credits",
        remaining: "Remaining",
        generatedAt: "Generated At",
        duration: "Duration",
      },
      durationUnit: "s",
      notAvailable: "--",
      channel: "Channel: {{channel}}",
      model: "Model: {{model}}",
    },
    usage: {
      title: "API Usage Records",
      empty: "No API usage records",
      status: {
        success: "Success",
        failed: "Failed",
        pending: "Pending",
      },
      refunded: "Refunded {{credits}} credits",
      deducted: "-{{credits}} credits",
    },
    serviceTypes: {
      gemini3ProImage: "Gemini 3 Pro Image",
      gemini31Image: "Nano banana 2 Image",
      gemini25Image: "Gemini 2.5 Image",
      geminiImageEdit: "Image Editing",
      gemini31ImageEdit: "Nano banana 2 Image Edit",
      geminiImageBlend: "Image Blending",
      gemini31ImageBlend: "Nano banana 2 Blend",
      geminiImageAnalyze: "Image Analysis",
      geminiText: "Text Chat",
      geminiPromptOptimize: "Prompt Optimization",
      geminiPaperJs: "Paper.js Generation",
      midjourneyImagine: "Midjourney Image",
      midjourneyVariation: "Midjourney Variation",
      backgroundRemoval: "Background Removal",
      expandImage: "Image Expansion",
      convert2dTo3d: "2D to 3D",
      soraSd: "Sora SD Video",
      soraHd: "Sora HD Video",
    },
  },
  workspacePage: {
    loadFailed: "Failed to load",
    prompt: {
      projectName: "Enter project name",
      defaultName: "Untitled Project",
    },
    user: {
      secondaryId: "153****6774 · Pro Plan",
    },
    quickActions: {
      title: "Quick Actions",
      desc: "Open or manage projects and review activity.",
      enterCanvas: "Open Canvas",
      newProject: "New Project",
    },
    recentProjects: {
      title: "Recent Projects",
      empty: 'No projects yet. Click "New Project" to start creating.',
    },
  },
  chat: {
    common: {
      auto: "Auto",
    },
    labels: {
      selectMode: "Select Mode",
      domesticModel: "Domestic Model",
      quickSwitchDomesticModel: "Quick switch domestic model",
      quickSwitchModel: "Quick Model Switch",
    },
    provider: {
      fastDesc: "Domestic fast model",
      proDesc: "Domestic Pro model",
      ultraDesc: "Domestic Ultra model",
    },
    manualMode: {
      autoDesc: "Automatically select the best tool",
      textDesc: "Text chat mode",
      generateDesc: "Image generation mode",
      editDesc: "Image editing mode",
      blendDesc: "Image blending mode",
      analyzeDesc: "Image analysis mode",
      videoDesc: "Video generation mode",
      vectorDesc: "Vector graphics mode",
      architectureDesc: "Architectural design AI agent",
    },
    placeholder: {
      text: "Ask anything or start chatting. AI will reply instantly.",
      blend: "Describe how to blend these {{count}} images...",
      edit: "Describe what you want to do. AI will decide between edit and analysis...",
      analyze: "Ask questions about this image, or leave empty for full analysis...",
      videoWithSource:
        "Describe the video effect you want. AI will generate a video from the uploaded image...",
      video: "Describe the scene, style, and motion you want in the video...",
      vector:
        "Describe the vector graphic you want, e.g. 'a blue pentagram' or 'concentric circles'...",
      generate: "Describe the scene, style, or details you want to generate...",
      default: "Type anything. AI will decide between image generation, chat, or video...",
    },
  },
  app: {
    authChecking: "Verifying login status...",
    mobile: {
      title: "Please Use Desktop",
      desc1: "TAI is a professional design tool and works best on desktop.",
      desc2: "Please open this website in a browser on PC or Mac.",
      recommendation: "Recommended browsers: Chrome, Edge, or Safari",
    },
  },
  export: {
    watermark: {
      title: "Export Image",
      description: "Choose whether to add a watermark to the exported image.",
      withWatermark: "Export with watermark",
      withoutWatermark: "Export without watermark",
      cancel: "Cancel",
    },
  },
  enterprise: {
    home: {
      title: "TAI Enterprise",
      subtitle: "Enterprise accounts are provisioned by the platform. Open the console of an enterprise you've joined, or apply with an invite code.",
      myEnterprises: "My Enterprises",
      enterConsole: "Open Console",
      noAccess: "No Enterprise Access",
      noAccessHint: "Please contact TAI to enable Enterprise for your account, or apply with an invite code from your admin.",
      inviteTitle: "Apply with Invite Code",
      invitePlaceholder: "Enter invite code or paste invite link",
      submit: "Submit",
      submitting: "Submitting…",
      successDefault: "Application submitted. Please wait for the admin to review.",
      backHome: "Back to Home",
    },
    login: {
      title: "Enterprise Sign In",
      subtitle: "Please sign in with your enterprise account",
      phone: "Phone",
      phonePlaceholder: "Phone number",
      password: "Password",
      passwordPlaceholder: "Password",
      submit: "Sign In",
      submitting: "Signing in…",
      backHome: "Back to Home",
      invalid: "Invalid phone or password, or your account is not authorized for the enterprise console.",
    },
    header: {
      console: "Enterprise Console",
      consoleTitle: "Open Enterprise Console",
      enterprise: "Enterprise",
      enterpriseTitle: "Open Enterprise",
      inviteCode: "Invite Code",
      inviteCodeTitle: "Enter Enterprise Invite Code",
      teamQuota: "Team Quota",
      unlimited: "Unlimited",
      teamQuotaHint: "Using team quota. The number shown is your personal available allowance.",
    },
    homeActions: {
      enterprise: "Enterprise",
      enterpriseContact: "Enterprise Inquiries",
      enterpriseContactDesc: "Enterprise\nSupport WeChat",
      enterpriseContactLabel: "Enterprise Support WeChat",
    },
  },
} as const;

export default enUS;
