...
- Linglong Seedance: 仅开放 **1.5 Pro**（`TIANYI_SEEDANCE_15_MODEL`，默认 `doubao-seedance-1-5-pro-251215`）；隐藏 2.x UI/节点，后端强制 1.5-pro，不再误用 `TIANYI_SEEDANCE_20_MODEL`。
...
- AI Gateway: removed the standalone `apps/new-api` service and the business-backend `new-api` gateway integration; `Omni Flash Ext` now defaults to direct `APIMart`, and `volc-enhance-video` public endpoints were removed with the gateway teardown.

### Added
- Flow/AI Video: added the async `Video Enhance` node (`videoEnhance`) plus dedicated backend `POST/GET /api/ai/volc-enhance-video` endpoints, dynamic version/resolution/FPS pricing, pre-deduct/refund/success credit settlement, `new_api` task submission/query normalization, frontend polling/history/progress UI, and admin node-config registration.
- New API / Tencent VOD: `apps/new-api` now supports managed `volc-enhance-video` through the existing `tencent_vod` adapter. Submission uses `PullUpload + Procedure` URL enhancement flow, polling reuses `DescribeTaskDetail`, and mapping config can resolve `toolVersion/scene/resolution/fps` to a VOD procedure name via `defaultProcedure` or nested `procedureMap`.
- New API / Tencent VOD: added `PATCH /admin/model-mappings/:id/config` plus `npm run volc-enhance:update-map` and a sample `volc-enhance-procedure-map.example.json`, so `volc-enhance-video` task-flow mappings can be maintained without editing the database directly.
...
- Profile completion: added required **company** (`公司`) field to extended profile form, API, and `User.profileCompany` column.
...
- Auth UI: restored the WeChat Official Account QR login tab on `/auth/login`; the page again defaults to the WeChat tab and resumes session create/poll/refresh/bind/consume handling from the existing auth API (`frontend/src/pages/auth/Login.tsx`).
...
### Fixed
- New API / Tencent VOD: `volc-enhance-video` polling now follows `PullUploadTask.ProcedureTaskId` into the real VOD procedure task instead of staying pinned to the initial upload task, so once `AIGC-超分增强` is triggered the gateway can observe the actual enhance status/result.
- Flow/Video Enhance: backend `GET /api/ai/volc-enhance-video/:taskId` now auto-closes timed-out enhance tasks after the shared 30-minute polling window, marks the corresponding `apiUsage` as failed, and triggers idempotent credit refund so frontend polling no longer spins forever on stuck upstream `PullUpload`-only jobs.
...
- Flow/Video Compose: the editor now waits for each clip's decode runtime and timeline thumbnails before leaving the loading state, so the compose UI no longer exposes a half-ready preview area on first open (`frontend/src/components/flow/nodes/VideoComposeEditorModal.tsx`).
- Flow/Video Compose: preview playback now stays on the canvas render path from the first visible frame, removes the initial `<video>` fallback swap, avoids clearing the canvas before every decoded frame, and raises preview refresh cadence to reduce black flashing during playback (`frontend/src/components/flow/nodes/VideoComposeEditorModal.tsx`).
- Flow/Video Compose: undo/redo history snapshots now keep shared thumbnail object URLs alive for the full editor session instead of revoking them during snapshot replacement, fixing the case where deleted clips came back from undo without thumbnails (`frontend/src/components/flow/nodes/VideoComposeEditorModal.tsx`).
- Flow/Video Compose: clip deletion now falls back to the selected clip, then the playhead clip, allows trimming the timeline down to a single remaining clip, preserves selection/current time after deletion, and shows warning toasts instead of silently doing nothing when deletion is unavailable (`frontend/src/components/flow/nodes/VideoComposeEditorModal.tsx`).
- Flow/Video Compose: external source normalization now skips `/api/uploads/transfer-video` for backend-allowed remote hosts and falls back to the original remote URL when transfer fails, so reopening the compose editor no longer hard-fails on repeated transfer attempts (`frontend/src/components/flow/nodes/videoCompose/ensureComposeSourceUrl.ts`).
- Flow/Video Compose / Assets Proxy: preview metadata loading no longer falls back to direct browser fetches for backend-allowed remote video hosts such as `getapib.org`, avoiding noisy CORS failures when proxy playback is the intended path; backend `/api/assets/proxy` now logs request start, redirects, retries, client aborts, and final upstream response headers for easier video debugging (`frontend/src/components/flow/nodes/VideoComposeEditorModal.tsx`, `backend/src/oss/assets.controller.ts`).
- Asset proxy: `proxifyRemoteAssetUrl(..., { forceProxy: true })` now truly forces proxying for ordinary video URLs as well, instead of returning direct `mp4/webm/m3u8` links on the early video branch; this fixes compose/video preview cases where `getapib.org` still bypassed `/api/assets/proxy` and hit browser CORS (`frontend/src/utils/assetProxy.ts`).
- Flow/Video Compose: node card copy and controls were cleaned up. The idle helper text `本地浏览器合成，不走服务端转码` and ready-state status text `仅拼接视频时间线` were removed, the result action button is now a more prominent `重新合成` pill with a refresh glyph, and the audio input handle size now matches the video handle (`frontend/src/components/flow/nodes/VideoComposeNode.tsx`).
