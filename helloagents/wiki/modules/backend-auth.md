...
- 公众号扫码登录依赖环境变量：`WECHAT_OFFICIAL_APP_ID`、`WECHAT_OFFICIAL_APP_SECRET`、`WECHAT_OFFICIAL_TOKEN`；安全模式/兼容模式回调依赖 `WECHAT_OFFICIAL_ENCODING_AES_KEY`，并兼容微信后台字段名 `EncodingAESKey`；可选 `WECHAT_OFFICIAL_QR_EXPIRE_SECONDS`、`WECHAT_OFFICIAL_LOGIN_MESSAGE`。
...
- 公众号回调支持明文模式和安全模式；若微信后台启用安全模式或兼容模式，需要确保 `EncodingAESKey` 已配置到后端环境变量。
