...
## 玲珑身份的赠送提醒
- 通过 `getDeploymentBrand()` 判断玲珑身份，隐藏顶部完善资料/签到赠送提醒、赠送积分入口与设置中的签到/邀请注册奖励面板，以及通用公告弹窗。
- 玲珑登录跳过完善资料送积分弹窗，直接进入目标页面；其他身份沿用原有流程。此处仅调整前端展示，不改变后端积分发放规则。

## 已移除功能
- PPT 工具栏入口、全部 `/ppt` 页面、大纲生成接口、逐页生图和 PDF 导出已移除；旧页面地址按通用未知路由跳回首页。
- tgAgent 的 PPT 编排工具、模板和事件协议同步移除。
